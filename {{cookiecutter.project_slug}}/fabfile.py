...
def comment_and_close_on_jira(c):
    ...
    with c.conn.cd(c.conn.project_root):
        remote_version = c.conn.git("rev-parse HEAD", hide=True).strip()
        new_version = subprocess.run("git rev-parse HEAD".split(" "), capture_output=True).stdout.strip()

    subprocess.run(
        f"jira_release {command} {jira_prefix} "
        f"{c.config.environment} {remote_version} {new_version} {os.getcwd()}".split(" "), capture_output=True)
